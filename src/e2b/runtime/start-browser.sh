...
LOG_DIR="/var/log/e2b"
PROFILE_DIR="${USER_HOME}/.cache/chudcode/browser-profile"
RUNTIME_DIR="${USER_HOME}/.cache/chudcode/runtime"
URL_FILE="${USER_HOME}/.cache/chudcode/browser-target-url"
TARGET_PORT_FILE="${USER_HOME}/.cache/chudcode/browser-target-port"
...
mkdir -p "${LOG_DIR}" "${PROFILE_DIR}" "${RUNTIME_DIR}" "${USER_HOME}/.cache/chudcode"
chown -R "${USER_NAME}:${USER_NAME}" "${USER_HOME}/.cache/chudcode"
rm -f "${READY_FILE}"
