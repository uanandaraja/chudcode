...
DESKTOP_USER="${E2B_DESKTOP_USER:-werkbench}"
...
cat > "${BIN_DIR}/werkbench-launch-browser.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
export CHROME_LOG="${HOME}/.cache/werkbench/chrome.log"

mkdir -p "${XDG_RUNTIME_DIR}" "${HOME}/.cache/werkbench/chrome-profile"
chmod 700 "${XDG_RUNTIME_DIR}"

if pgrep -u "${USER}" -f "${HOME}/.cache/werkbench/chrome-profile" >/dev/null 2>&1; then
  exit 0
fi
...
nohup google-chrome-stable \
  ...
  --user-data-dir="${HOME}/.cache/werkbench/chrome-profile" \
...
cat > "${BIN_DIR}/werkbench-focus-browser.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
if ! pgrep -u "${USER}" -f "${HOME}/.cache/werkbench/chrome-profile" >/dev/null 2>&1; then
  nohup "${HOME}/.local/bin/werkbench-launch-browser.sh" >/dev/null 2>&1 &
fi
...
chmod 755 "${BIN_DIR}/werkbench-launch-browser.sh"
chmod 755 "${BIN_DIR}/werkbench-focus-browser.sh"
chown "${USER_NAME}:${USER_NAME}" "${BIN_DIR}/werkbench-launch-browser.sh" "${BIN_DIR}/werkbench-focus-browser.sh"

cat > "${AUTOSTART_DIR}/werkbench-browser.desktop" <<EOF
[Desktop Entry]
Type=Application
Version=1.0
Name=Werkbench Browser
Exec=${BIN_DIR}/werkbench-launch-browser.sh
...
chown "${USER_NAME}:${USER_NAME}" "${VNC_DIR}/kasmvnc.yaml" "${VNC_DIR}/xstartup" "${AUTOSTART_DIR}/werkbench-browser.desktop"
...
nohup sudo -u "${USER_NAME}" env \
  ...
  bash -lc "${BIN_DIR}/werkbench-focus-browser.sh" \
